...
#define HANDLER_NAME "videoRedirectTable"

using namespace std;

static vector<videoRedirectTable_t> g_videoRedirectTable;
...
//********************************************************************************
// Function: init_videoRedirectTable
//
// Description: videoRedirectTable initialization for mib.
//********************************************************************************
bool init_videoRedirectTable(void)
{
    syslog(LOG_DEBUG, "Enter: %s", __FUNCTION__); 

    static oid videoRedirectTableOID[] = { 1, 3, 6, 1, 4, 1, 1457, 4, 1, 1, 8 };

    netsnmp_handler_registration * reg{};
    reg = netsnmp_create_handler_registration(HANDLER_NAME,
        handler, videoRedirectTableOID, OID_LENGTH(videoRedirectTableOID),
        HANDLER_CAN_RONLY);
    ...
    table_info->min_column = COLUMN_VIDEO_REDIRECT;
    table_info->max_column = COLUMN_VIDEO_REDIRECT_COUNT;
...
//
// Description: Returns first data point of videoRedirectTable.
//********************************************************************************
...
{
    load_table();
    if(!g_videoRedirectTable.size())
    {
        *data_context = (void*) 0;
        *loop_context = (void*) 0;
        return NULL;
    }
    *loop_context = (void*) g_videoRedirectTable.data();
...
//
// Description: Returns next data point of videoRedirectTable.
//********************************************************************************
static netsnmp_variable_list *
    ...
    netsnmp_variable_list * put_index_data, netsnmp_iterator_info * data)
{
    ...
    videoRedirectTable_t *  pd = (videoRedirectTable_t *) *loop_context;
    videoRedirectTable_t * end = g_videoRedirectTable.data();
    end += g_videoRedirectTable.size();
    if(pd == end)
        return NULL;

    netsnmp_variable_list *idx = put_index_data;
    snmp_set_var_typed_integer(idx, ASN_INTEGER, pd->videoRedirectIdx);
...
// Function: handler
//
// Description: Handle videoRedirectTable get request.
//********************************************************************************
...
static void delayed_response(unsigned int clientreg, void * clientarg)
{
    ...
    {
        videoRedirectTable_t * v =
            (videoRedirectTable_t *) netsnmp_extract_iterator_context(request);
        if(!v)
        {
            netsnmp_set_request_error(cache->reqinfo, request, SNMP_NOSUCHINSTANCE);
            g_videoRedirectTable.clear();
            continue;
        }
        ...
        {
            case COLUMN_VIDEO_REDIRECT:
            snmp_set_var_typed_value(request->requestvb, ASN_OCTET_STR,
                (unsigned char*) v->videoRedirect, strlen(v->videoRedirect));
            break;

            case COLUMN_VIDEO_REDIRECT_COUNT:
            snmp_set_var_typed_integer(request->requestvb, ASN_COUNTER,
                v->videoRedirectCount);
            break;
...
// Function: load_table
//
// Description: Pull elements from tcache_plane into videoRedirectTable.
//********************************************************************************
static void load_table(void)
{
    if(g_videoRedirectTable.size())
    ...
    sprintf(buffer, "%d", videoRedirectTable);
    ...
    while(true)
    {
        ...
        {
            videoRedirectTable_t v;
            v.videoRedirectIdx = idx++;
            strcpy(v.videoRedirect, key);
            v.videoRedirectCount = atoi(value);
            g_videoRedirectTable.push_back(v);
        }
        size = sizeof(size_t);
        int rcvmore{};
