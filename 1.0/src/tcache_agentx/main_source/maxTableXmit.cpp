...
#define HANDLER_NAME "maxTableXmit"

static Netsnmp_Node_Handler maxTableXmit_handler;

static bool set_maxTableXmit(uint32_t count);

static uint32_t get_maxTableXmit(void);

static void delayed_response(unsigned int clientreg, void * clientarg);

//********************************************************************************
// Function: init_maxTableXmit
//
// Description: Control Plane maxTableXmit initialization for mib.
//********************************************************************************
bool init_maxTableXmit(void)
{
    syslog(LOG_DEBUG, "Enter: %s.", __FUNCTION__);

    static oid statusOID[] = { 1, 3, 6, 1, 4, 1, 1457, 4, 1, 1, 13 };

    netsnmp_handler_registration * reg{};
    reg = netsnmp_create_handler_registration(HANDLER_NAME, maxTableXmit_handler,
        statusOID, OID_LENGTH(statusOID), HANDLER_CAN_RWRITE);
    if(!reg)
    {
...
//********************************************************************************
// Function: maxTableXmit_handler
//
// Description: Retrieve or Set Max Table Transmision Value.
//********************************************************************************
static int maxTableXmit_handler(netsnmp_mib_handler * handler,
...
static void delayed_response(unsigned int clientreg, void * clientarg)
{
    ...
    uint32_t maxTableXmit{};
    uint32_t * dupCache{};
    switch(cache->reqinfo->mode)
    {
        case MODE_GET:
        case MODE_GETNEXT:
        maxTableXmit = get_maxTableXmit();
        snmp_set_var_typed_integer(cache->requests->requestvb, ASN_UNSIGNED, maxTableXmit);
        break;
        ...
        memdup((uint8_t **) &dupCache,
            (uint8_t *) &maxTableXmit, sizeof(maxTableXmit));
        if(!dupCache)
        {
        ...
        maxTableXmit =  *(cache->requests->requestvb->val.integer);
        (void) set_maxTableXmit(maxTableXmit);
        syslog(LOG_DEBUG, "%s: updated maxTableXmit: %d", HANDLER_NAME, maxTableXmit);
        break;

        case MODE_SET_UNDO:
        maxTableXmit = *((uint32_t *) netsnmp_request_get_list_data(cache->requests,
            HANDLER_NAME));
        break;
...
//********************************************************************************
// Function: get_maxTableXmit
//
// Description: Retrieve maxTableXmit value from Control Plane.
//********************************************************************************
static uint32_t get_maxTableXmit(void)
{
    ...
    sprintf(buffer, "%d", getMaxTableXmit);
...
//********************************************************************************
// Function: set_maxTableXmit
//
// Description: Set maxTableXmit value int Control Plane.
//********************************************************************************
static bool set_maxTableXmit(uint32_t count)
{
    void *context = zmq_ctx_new();
    ...
    sprintf(buffer, "%d|%u", setMaxTableXmit, count);
    zmq_send(requester, buffer, strlen(buffer), 0);
