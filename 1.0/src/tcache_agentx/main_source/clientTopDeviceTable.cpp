...
#define HANDLER_NAME "clientTopDeviceTable"

using namespace std;

static vector<clientTopDeviceTable_t> g_clientTopDeviceTable;
...
//********************************************************************************
// Function: init_clientTopDeviceTable
//
// Description: clientTopDeviceTable initialization for mib.
//********************************************************************************
bool init_clientTopDeviceTable(void)
{
    syslog(LOG_DEBUG, "Enter: %s", __FUNCTION__); 

    static oid clientTopDeviceTableOID[] = { 1, 3, 6, 1, 4, 1, 1457, 4, 1, 1, 11 };

    netsnmp_handler_registration * reg{};
    reg = netsnmp_create_handler_registration(HANDLER_NAME,
        handler, clientTopDeviceTableOID,
        OID_LENGTH(clientTopDeviceTableOID), HANDLER_CAN_RONLY);
    ...
    table_info->min_column = COLUMN_CLIENT_TOPDEVICE;
    table_info->max_column = COLUMN_CLIENT_TOPDEVICE_COUNT;
...
//
// Description: Returns first data point of clientTopDeviceTable.
//********************************************************************************
...
{
    load_table();
    if(!g_clientTopDeviceTable.size())
    {
        *data_context = (void*) 0;
        *loop_context = (void*) 0;
        return NULL;
    }
    *loop_context = (void*) g_clientTopDeviceTable.data();
...
//
// Description: Returns next data point of clientTopDeviceTable.
//********************************************************************************
static netsnmp_variable_list *
    ...
    netsnmp_variable_list * put_index_data, netsnmp_iterator_info * data)
{
    ...
    clientTopDeviceTable_t *  pd = (clientTopDeviceTable_t *) *loop_context;
    clientTopDeviceTable_t * end = g_clientTopDeviceTable.data();
    end += g_clientTopDeviceTable.size();
    if(pd == end)
        return NULL;

    netsnmp_variable_list *idx = put_index_data;
    snmp_set_var_typed_integer(idx, ASN_INTEGER, pd->clientTopDeviceIdx);
...
// Function: handler
//
// Description: Handle clientTopDeviceTable get request.
//********************************************************************************
...
static void delayed_response(unsigned int clientreg, void * clientarg)
{
    ...
    {
        clientTopDeviceTable_t * c =
            (clientTopDeviceTable_t *) netsnmp_extract_iterator_context(request);
        if(!c)
        {
            netsnmp_set_request_error(cache->reqinfo, request, SNMP_NOSUCHINSTANCE);
            g_clientTopDeviceTable.clear();
            continue;
        }
        ...
        {
            case COLUMN_CLIENT_TOPDEVICE:
            snmp_set_var_typed_value(request->requestvb, ASN_OCTET_STR,
                (unsigned char*) c->clientTopDevice, strlen(c->clientTopDevice));
            break;

            case COLUMN_CLIENT_TOPDEVICE_COUNT:
            snmp_set_var_typed_integer(request->requestvb, ASN_COUNTER,
                c->clientTopDeviceCount);
            break;
...
// Function: load_table
//
// Description: Pull elements from tcache_plane into clientTopDeviceTable.
//********************************************************************************
static void load_table(void)
{
    if(g_clientTopDeviceTable.size())
    ...
    sprintf(buffer, "%d", clientTopDeviceTable);
    ...
    while(true)
    {
        ...
        {
            clientTopDeviceTable_t c;
            c.clientTopDeviceIdx = idx++;
            strcpy(c.clientTopDevice, key);
            c.clientTopDeviceCount = atoi(value);
            g_clientTopDeviceTable.push_back(c);
        }
        size = sizeof(size_t);
        int rcvmore{};
