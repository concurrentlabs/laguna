...
#define HANDLER_NAME "trafficCount"

static Netsnmp_Node_Handler traffic_handler;

static int32_t traffic_count(void);

static void delayed_response(unsigned int clientreg, void * clientarg);

//********************************************************************************
// Function: init_traffic_count
//
// Description: Control Plane traffic count initialization for mib.
//********************************************************************************
bool init_trafficCount(void)
{
    syslog(LOG_DEBUG, "Enter: %s", __FUNCTION__);

    static oid statusOID[] = { 1, 3, 6, 1, 4, 1, 1457, 4, 1, 1, 3 };

    netsnmp_handler_registration * reg{};
    reg = netsnmp_create_handler_registration(HANDLER_NAME, traffic_handler, statusOID,
...
//********************************************************************************
// Function: traffic_handler
//
// Description: Retrieve traffic count from Control Plane.
//********************************************************************************
static int traffic_handler(netsnmp_mib_handler * handler,
...
//
// Description: Recieve Control Plane traffic count.
//********************************************************************************
static int32_t traffic_count(void)
{
