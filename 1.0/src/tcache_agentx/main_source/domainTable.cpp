...
#define HANDLER_NAME "domainTable"

using namespace std;

static vector<domainTable_t> g_domainTable;
...
//********************************************************************************
// Function: init_domainTable
//
// Description: domainTable initialization for mib.
//********************************************************************************
bool init_domainTable(void)
{
    syslog(LOG_DEBUG, "Enter: %s", __FUNCTION__); 

    static oid domainTableOID[] = { 1, 3, 6, 1, 4, 1, 1457, 4, 1, 1, 5 };

    netsnmp_handler_registration * reg{};
    reg = netsnmp_create_handler_registration(HANDLER_NAME,
        handler, domainTableOID, OID_LENGTH(domainTableOID),
        HANDLER_CAN_RONLY);
    ...
    table_info->min_column = COLUMN_DOMAIN;
    table_info->max_column = COLUMN_DOMAIN_COUNT;
...
//
// Description: Returns first data point of domainTable.
//********************************************************************************
...
{
    load_table();
    if(!g_domainTable.size())
    {
        *data_context = (void*) 0;
        *loop_context = (void*) 0;
        return NULL;
    }
    *loop_context = (void*) g_domainTable.data();
...
//
// Description: Returns next data point of domainTable.
//********************************************************************************
static netsnmp_variable_list *
    ...
    netsnmp_variable_list * put_index_data, netsnmp_iterator_info * data)
{
    ...
    domainTable_t *  pd = (domainTable_t *) *loop_context;
    domainTable_t * end = g_domainTable.data(); end += g_domainTable.size();
    if(pd == end)
        return NULL;

    netsnmp_variable_list *idx = put_index_data;
    snmp_set_var_typed_integer(idx, ASN_INTEGER, pd->domainIdx);
...
// Function: handler
//
// Description: Handle domainTable get request.
//********************************************************************************
...
static void delayed_response(unsigned int clientreg, void * clientarg)
{
    ...
    {
        domainTable_t * d =
            (domainTable_t *) netsnmp_extract_iterator_context(request);
        if(!d)
        {
        ...
        switch (table_info->colnum)
        {
            ...
            snmp_set_var_typed_value(request->requestvb, ASN_OCTET_STR,
                (unsigned char*) d->domain, strlen(d->domain));
            break;

            case COLUMN_DOMAIN_COUNT:
...
// Function: load_table
//
// Description: Pull elements from tcache_plane into domainTable.
//********************************************************************************
static void load_table(void)
{
    if(g_domainTable.size())
    ...
    while(true)
    {
        ...
        if(value)
        {
            ...
            d.domainIdx = idx++;
            strcpy(d.domain, key);
            d.domainCount = atoi(value);
            g_domainTable.push_back(d);
        }
        size = sizeof(size_t);
        int rcvmore{};
