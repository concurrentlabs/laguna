...
#define HANDLER_NAME "tcPlaneStatus"

static Netsnmp_Node_Handler status_handler;

static bool ping_tcPlane(void);

static void delayed_response(unsigned int clientreg, void * clientarg);

//********************************************************************************
// Function: init_tcPlane_status
//
// Description: Control Plane status initialization for mib.
//********************************************************************************
bool init_tcPlaneStatus(void)
{
    syslog(LOG_DEBUG, "Enter: %s", __FUNCTION__);

    static oid statusOID[] = { 1, 3, 6, 1, 4, 1, 1457, 4, 1, 1, 2 };

    netsnmp_handler_registration * reg{};
    reg = netsnmp_create_handler_registration(HANDLER_NAME, status_handler, statusOID,
    ...
    {
        syslog(LOG_CRIT, "%s", "ERROR: netsnmp_regester_scalar.");
        return false;
    }
...
//
// Description: Ping Control Plane.
//********************************************************************************
static int status_handler(netsnmp_mib_handler * handler,
...
static void delayed_response(unsigned int clientreg, void * clientarg)
{
    ...
    switch(cache->reqinfo->mode)
    {
        ...
        case MODE_GETNEXT:
        if(ping_tcPlane())
            snmp_set_var_typed_integer(cache->requests->requestvb, ASN_COUNTER, 1);
        else
            snmp_set_var_typed_integer(cache->requests->requestvb, ASN_COUNTER, 0);
    }
