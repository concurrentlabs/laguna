...
#define HANDLER_NAME "videoTable"

using namespace std;

static vector<videoTable_t> g_videoTable;
...
//********************************************************************************
// Function: init_videoTable
//
// Description: videoTable initialization for mib.
//********************************************************************************
bool init_videoTable(void)
{
    syslog(LOG_DEBUG, "Enter: %s", __FUNCTION__); 

    static oid videoTableOID[] = { 1, 3, 6, 1, 4, 1, 1457, 4, 1, 1, 7 };

    netsnmp_handler_registration * reg{};
    reg = netsnmp_create_handler_registration(HANDLER_NAME,
        handler, videoTableOID, OID_LENGTH(videoTableOID),
        HANDLER_CAN_RONLY);
    ...
    table_info->min_column = COLUMN_VIDEO;
    table_info->max_column = COLUMN_VIDEO_COUNT;
...
//
// Description: Returns first data point of videoTable.
//********************************************************************************
...
{
    load_table();
    if(!g_videoTable.size())
    {
        *data_context = (void*) 0;
        *loop_context = (void*) 0;
        return NULL;
    }
    *loop_context = (void*) g_videoTable.data();
...
//
// Description: Returns next data point of videoTable.
//********************************************************************************
static netsnmp_variable_list *
    ...
    netsnmp_variable_list * put_index_data, netsnmp_iterator_info * data)
{
    ...
    videoTable_t *  pd = (videoTable_t *) *loop_context;
    videoTable_t * end = g_videoTable.data(); end += g_videoTable.size();
    if(pd == end)
        return NULL;

    netsnmp_variable_list *idx = put_index_data;
    snmp_set_var_typed_integer(idx, ASN_INTEGER, pd->videoIdx);
...
// Function: handler
//
// Description: Handle videoTable get request.
//********************************************************************************
...
static void delayed_response(unsigned int clientreg, void * clientarg)
{
    ...
    {
        videoTable_t * v =
            (videoTable_t *) netsnmp_extract_iterator_context(request);
        if(!v)
        {
        ...
        switch(table_info->colnum)
        {
            ...
            snmp_set_var_typed_value(request->requestvb, ASN_OCTET_STR,
                (unsigned char*) v->video, strlen(v->video));
            break;

            case COLUMN_VIDEO_COUNT:
...
// Function: load_table
//
// Description: Pull elements from tcache_plane into videoTable.
//********************************************************************************
static void load_table(void)
{
    if(g_videoTable.size())
    ...
    while(true)
    {
        ...
        if(value)
        {
            ...
            v.videoIdx = idx++;
            strcpy(v.video, key);
            v.videoCount = atoi(value);
            g_videoTable.push_back(v);
        }
        size = sizeof(size_t);
        int rcvmore{};
