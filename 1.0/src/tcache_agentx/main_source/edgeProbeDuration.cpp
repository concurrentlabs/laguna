...
#define HANDLER_NAME "edgeProbeDuration"

static Netsnmp_Node_Handler edgeProbeDuration_handler;

static bool set_edgeProbeDuration(uint32_t duration);

static uint32_t get_edgeProbeDuration(void);

static void delayed_response(unsigned int clientreg, void * clientarg);

//********************************************************************************
// Function: init_edgeProbeDuration
//
// Description: Control Plane edgeProbeDuration initialization for mib.
//********************************************************************************
bool init_edgeProbeDuration(void)
{
    syslog(LOG_DEBUG, "Enter: %s.", __FUNCTION__);

    static oid statusOID[] = { 1, 3, 6, 1, 4, 1, 1457, 4, 1, 1, 14 };

    netsnmp_handler_registration * reg{};
    reg = netsnmp_create_handler_registration(HANDLER_NAME,
        edgeProbeDuration_handler, statusOID, OID_LENGTH(statusOID),
        HANDLER_CAN_RWRITE);
    if(!reg)
    {
...
//********************************************************************************
// Function: edgeProbeDuration_handler
//
// Description: Retrieve or Set Edge Probe Duration Value.
//********************************************************************************
static int edgeProbeDuration_handler(netsnmp_mib_handler * handler,
...
static void delayed_response(unsigned int clientreg, void * clientarg)
{
    ...
    uint32_t edgeProbeDuration{};
    uint32_t * dupCache{};
    switch(cache->reqinfo->mode)
    {
        case MODE_GET:
        case MODE_GETNEXT:
        edgeProbeDuration = get_edgeProbeDuration();
        ...
        memdup((uint8_t **) &dupCache,
            (uint8_t *) &edgeProbeDuration, sizeof(edgeProbeDuration));
        if(!dupCache)
        {
        ...
        case MODE_SET_ACTION:
         edgeProbeDuration =  *(cache->requests->requestvb->val.integer);
        (void) set_edgeProbeDuration(edgeProbeDuration);
        syslog(LOG_DEBUG, "%s: updated edgeProbeDuration: %d", HANDLER_NAME,
            edgeProbeDuration);
        break;

        case MODE_SET_UNDO:
        edgeProbeDuration = *((uint32_t *) netsnmp_request_get_list_data(cache->requests,
            HANDLER_NAME));
        break;
...
//********************************************************************************
// Function: get_edgeProbeDuration
//
// Description: Retrieve get_edgeProbeDuration value from Control Plane.
//********************************************************************************
static uint32_t get_edgeProbeDuration(void)
{
    ...
    sprintf(buffer, "%d", getEdgeProbeDuration);
...
//********************************************************************************
// Function: set_edgeProbeDuration
//
// Description: Set set_edgeProbeDuration value int Control Plane.
//********************************************************************************
static bool set_edgeProbeDuration(uint32_t duration)
{
    void *context = zmq_ctx_new();
    ...
    sprintf(buffer, "%d|%u", setEdgeProbeDuration, duration);
    zmq_send(requester, buffer, strlen(buffer), 0);
