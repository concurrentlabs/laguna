...
#define HANDLER_NAME "clientRedirectTable"

using namespace std;

static vector<clientRedirectTable_t> g_clientRedirectTable;
...
//********************************************************************************
// Function: init_clientRedirectTable
//
// Description: clientRedirectTable initialization for mib.
//********************************************************************************
bool init_clientRedirectTable(void)
{
    syslog(LOG_DEBUG, "Enter: %s.", __FUNCTION__); 

    static oid clientRedirectTableOID[] = { 1, 3, 6, 1, 4, 1, 1457, 4, 1, 1, 10 };

    netsnmp_handler_registration * reg{};
    reg = netsnmp_create_handler_registration(HANDLER_NAME,
        handler, clientRedirectTableOID,
        OID_LENGTH(clientRedirectTableOID), HANDLER_CAN_RONLY);
    ...
    table_info->min_column = COLUMN_CLIENT_REDIRECT;
    table_info->max_column = COLUMN_CLIENT_REDIRECT_COUNT;
...
//
// Description: Returns first data point of clientRedirectTable.
//********************************************************************************
...
{
    load_table();
    if(!g_clientRedirectTable.size())
    {
        *data_context = (void*) 0;
        *loop_context = (void*) 0;
        return NULL;
    }
    *loop_context = (void*) g_clientRedirectTable.data();
...
//
// Description: Returns next data point of clientRedirectTable.
//********************************************************************************
static netsnmp_variable_list *
    ...
    netsnmp_variable_list * put_index_data, netsnmp_iterator_info * data)
{
    ...
    clientRedirectTable_t *  pd = (clientRedirectTable_t *) *loop_context;
    clientRedirectTable_t * end = g_clientRedirectTable.data();
    end += g_clientRedirectTable.size();
    if(pd == end)
        return NULL;

    netsnmp_variable_list *idx = put_index_data;
    snmp_set_var_typed_integer(idx, ASN_INTEGER, pd->clientRedirectIdx);
...
// Function: handler
//
// Description: Handle clientRedirectTable get request.
//********************************************************************************
...
static void delayed_response(unsigned int clientreg, void * clientarg)
{
    ...
    {
        clientRedirectTable_t * c =
            (clientRedirectTable_t *) netsnmp_extract_iterator_context(request);
        if(!c)
        {
            netsnmp_set_request_error(cache->reqinfo, request, SNMP_NOSUCHINSTANCE);
            g_clientRedirectTable.clear();
            continue;
        }
        ...
        {
            case COLUMN_CLIENT_REDIRECT:
            snmp_set_var_typed_value(request->requestvb, ASN_OCTET_STR,
                (unsigned char*) c->clientRedirect, strlen(c->clientRedirect));
            break;

            case COLUMN_CLIENT_REDIRECT_COUNT:
            snmp_set_var_typed_integer(request->requestvb, ASN_COUNTER,
                c->clientRedirectCount);
            break;
...
// Function: load_table
//
// Description: Pull elements from tcache_plane into clientRedirectTable.
//********************************************************************************
static void load_table(void)
{
    if(g_clientRedirectTable.size())
    ...
    sprintf(buffer, "%d", clientRedirectTable);
    ...
    while(true)
    {
        ...
        {
            clientRedirectTable_t c;
            c.clientRedirectIdx = idx++;
            strcpy(c.clientRedirect, key);
            c.clientRedirectCount = atoi(value);
            g_clientRedirectTable.push_back(c);
        }
        size = sizeof(size_t);
        int rcvmore{};
