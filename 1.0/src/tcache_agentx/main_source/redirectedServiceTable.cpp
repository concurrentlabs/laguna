...
#define HANDLER_NAME "redirectedServiceTable"

using namespace std;

static vector<redirectedServiceTable_t> g_redirectedServiceTable;
...
//********************************************************************************
// Function: init_redirectedServiceTable
//
// Description: redirectedServiceTable initialization for mib.
//********************************************************************************
bool init_redirectedServiceTable(void)
{
    syslog(LOG_DEBUG, "Enter: %s", __FUNCTION__); 

    static oid redirectedServiceTableOID[] = { 1, 3, 6, 1, 4, 1, 1457, 4, 1, 1, 12 };

    netsnmp_handler_registration * reg{};
    reg = netsnmp_create_handler_registration(HANDLER_NAME,
        handler, redirectedServiceTableOID, OID_LENGTH(redirectedServiceTableOID),
        HANDLER_CAN_RONLY);
    ...
    table_info->min_column = COLUMN_REDIRECTED_SERVICE;
    table_info->max_column = COLUMN_REDIRECTED_SERVICE_COUNT;
...
//
// Description: Returns first data point of redirectedServiceTable.
//********************************************************************************
...
{
    load_table();
    if(!g_redirectedServiceTable.size())
    {
        *data_context = (void*) 0;
        *loop_context = (void*) 0;
        return NULL;
    }
    *loop_context = (void*) g_redirectedServiceTable.data();
...
//
// Description: Returns next data point of redirectedServiceTable.
//********************************************************************************
static netsnmp_variable_list *
    ...
    netsnmp_variable_list * put_index_data, netsnmp_iterator_info * data)
{
    ...
    redirectedServiceTable_t *  pd = (redirectedServiceTable_t *) *loop_context;
    redirectedServiceTable_t * end = g_redirectedServiceTable.data();
    end += g_redirectedServiceTable.size();
    if(pd == end)
        return NULL;

    netsnmp_variable_list *idx = put_index_data;
    snmp_set_var_typed_integer(idx, ASN_INTEGER, pd->redirectedServiceIdx);
...
// Function: handler
//
// Description: Handle redirectedServiceTable get request.
//********************************************************************************
...
static void delayed_response(unsigned int clientreg, void * clientarg)
{
    ...
    {
        redirectedServiceTable_t * r =
            (redirectedServiceTable_t *) netsnmp_extract_iterator_context(request);
        if(!r)
        {
            netsnmp_set_request_error(cache->reqinfo, request, SNMP_NOSUCHINSTANCE);
            g_redirectedServiceTable.clear();
            continue;
        }
        ...
        {
            case COLUMN_REDIRECTED_SERVICE:
            snmp_set_var_typed_value(request->requestvb, ASN_OCTET_STR,
                (unsigned char*) r->redirectedService, strlen(r->redirectedService));
            break;

            case COLUMN_REDIRECTED_SERVICE_COUNT:
            snmp_set_var_typed_integer(request->requestvb, ASN_COUNTER,
                r->redirectedServiceCount);
            break;
...
// Function: load_table
//
// Description: Pull elements from tcache_plane into redirectedServiceTable.
//********************************************************************************
static void load_table(void)
{
    if(g_redirectedServiceTable.size())
    ...
    sprintf(buffer, "%d", redirectedServiceTable);
    ...
    while(true)
    {
        ...
        {
            redirectedServiceTable_t r;
            r.redirectedServiceIdx = idx++;
            strcpy(r.redirectedService, key);
            r.redirectedServiceCount = atoi(value);
            g_redirectedServiceTable.push_back(r);
        }
        size = sizeof(size_t);
        int rcvmore{};
